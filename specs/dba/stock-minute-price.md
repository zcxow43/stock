...
4. **`volume` 為該分鐘的成交股數，非累計值。** 資料源部分端點回傳當日累計量；正規化階段必須轉為單根增量後才寫入。存累計值會讓成交量副圖畫成單調遞增的階梯而非柱狀圖。單位一律為**股**，與 `stock_daily_price.volume` 相同；來源若以「張」回報（富果即是），正規化階段必須 × 1000 後才寫入，否則同一欄位混著兩種單位，前端無從分辨。
