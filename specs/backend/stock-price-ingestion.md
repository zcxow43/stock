...
- **口徑差異的實際影響很小**：成交量不參與任何運算——MACD／KD 只讀價格，型態掃描（`specs/backend/strategy-scan.md`）與產業別漲幅排行（`specs/backend/industry-gain-ranking.md`）也不讀量——它只出現在顯示欄位（`latestVolume` / `totalVolume` / `avgVolume` 與日 K 圖的量副圖）。
