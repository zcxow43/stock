...
三大法人買賣超不在本模組內抓取，由 `specs/backend/institutional-trade-ingestion.md` 在本模組的每一次回補作業結束、以及每日增量成功後，於背景補齊。本模組的 API 回應、進度與行為都不因此改變。
...
| 全市場回補 | 交易所**逐日**全市場快照 | 每個交易日 **1 次** | 系統初次建置 |

**回補作業的請求成本單位是「交易日」，不是「檔數」。** 這是本模組最重要的一條成本性質，其他所有規範都建立在它之上：全市場回補一段區間所需的請求數等於該區間的交易日數，與母體有 34 檔還是 1,030 檔完全無關。一年份的區間約 165 個交易日、165 次請求；補一天就是 1 次請求。

「指定多檔」與「全市場」**採用不同的抓取策略，但共用同一套進度追蹤、速率控制、重試與 UPSERT 語意**。差別只在一件事——哪一種抓法的請求數比較少：

| 模式 | 抓法 | 一年份區間的請求數 |
|---|---|---|
| `SELECTED`（指名 2 檔） | 逐檔歷史查詢 | 2 |
| `ALL`（全市場 1,030 檔） | 逐日全市場快照 | 約 165 |

指名 2 檔時逐日快照要發 165 次請求才拿得到那 2 檔的資料，逐檔只要 2 次；全市場時逐檔要發 1,030 次，逐日只要 165 次。兩種抓法各自在自己的模式下是最省的那一個，所以兩條都保留，依模式選擇，**不是**其中一條取代另一條。

**逐檔歷史有兩個可互換的外部來源**（Yahoo 為主、FinMind 為備援），任一來源對本機 IP 施加封鎖時自動切換到另一個繼續作業。完整規範見下方「來源選擇與封鎖切換」。這套機制在逐日快照成為全市場回補的主路徑之後**仍然完整保留**，理由有二：`SELECTED` 模式本來就走逐檔，而 `ALL` 模式在逐日快照來源不可用時要靠它降級續跑（見「逐日快照不可用時降級為逐檔」）。
...
本模組有**四個**外部來源，職責不同，不可互相替代：

| 用途 | 來源 | 端點 | 涵蓋 | 請求數 |
|---|---|---|---|---|
| 每日全市場快照 | 交易所 OpenAPI | `https://openapi.twse.com.tw/v1/exchangeReport/STOCK_DAY_ALL` | 全體上市股票的**當日**一根日 K | 1 次 |
| 逐日全市場快照（**全市場回補主路徑**） | 交易所 MI_INDEX | `https://www.twse.com.tw/rwd/zh/afterTrading/MI_INDEX?date=<YYYYMMDD>&type=ALL&response=json` | 全體上市股票的**指定某一日**一根日 K | 每個交易日 1 次 |
...
它**沒有日期參數**，只給得到「最新一個已發布的交易日」。這就是它不能拿來回補歷史的原因，也是下方 MI_INDEX 存在的理由——兩者是同一家交易所的兩支端點，差別只在能不能指定日期。

#### 逐日全市場快照（交易所 MI_INDEX）— 已測事實

這是全市場回補的主路徑來源。以下皆為實測（`date=20260904`）確認：

- **`date` 可指定任何過去的交易日**，格式為 `YYYYMMDD`（西元，非民國）。這正是 `STOCK_DAY_ALL` 所缺的能力，也是把「1,030 次逐檔請求」換成「165 次逐日請求」的全部依據。
- 回應為 `{"tables":[...]}`，**內含多張表**（價格指數、報酬指數、成交統計、漲跌證券數、個股行情…）。個股行情是其中**唯一一張 `fields[0]` 為 `"證券代號"` 的表**——必須以這個條件定位它，**不得以陣列索引取表**：表的數量與順序會隨交易所版面調整而變動，以索引取表是一個不會拋錯、只會靜默取到錯誤資料的寫法。
- 該表 `fields` 為：`證券代號`／`證券名稱`／`成交股數`／`成交筆數`／`成交金額`／`開盤價`／`最高價`／`最低價`／`收盤價`／`漲跌(+/-)`／`漲跌價差`／`最後揭示買價`／`最後揭示買量`／`最後揭示賣價`／`最後揭示賣量`／`本益比`。`data` 為與 `fields` 逐一索引對應的字串陣列。
- 實測回傳 **1,093 檔四位數代號**的列，涵蓋整個上市普通股母體（約 1,030 檔）。
- **本來源同時提供 `成交金額` 與 `成交筆數`**，因此由它寫入的列 `turnover` 與 `transaction_count` 都是實際值，不是 Yahoo 那樣寫 `0`（見「價格處理」）。
- 價格與數量為**帶千分位符號的字串**，且可能出現非數值的佔位符。正規化必須沿用既有的 `NormalizeUtil` 千分位／佔位符處理，**不得另寫一套解析**。
- 欄位中的 `漲跌(+/-)` 夾帶 HTML（`<p style='color:red'>+</p>`）。本模組**不使用**漲跌相關欄位——寫入所需的一切都在開高低收與三個量值欄位裡——故不需要解析它。
- **非交易日**（週末、假日）：回應中不存在 `證券代號` 那張表，或其 `data` 為空陣列。這是**正常情形，不是錯誤**，處置見「逐日回補的處理流程」。

#### 逐檔歷史為什麼仍然必須有兩個來源

逐檔路徑仍是 `SELECTED` 模式的抓法，也是 `ALL` 模式在逐日快照不可用時的降級路徑，期間任一來源都可能對本機 IP 施加封鎖。已發生的實例（2026-09-06）：FinMind 回 `403 {"msg":"ip banned","status":403,"retry_after":505,"token_tail":""}`，該次啟動 131 檔全數失敗、**0 檔成功**；同一時間、同一 IP 的交易所快照是成功的（1368 檔主檔 upsert 完成），證實封鎖來自單一服務，而非連外中斷。**單一逐檔來源即單點故障**——這是雙來源存在的直接原因，與 `stock_sync_progress` 斷點續傳存在的原因（逐檔、長時間、會中斷）是同一個。
...
並行度與間隔管的是不同的事，不可互相替代：**間隔**決定來源感受到的請求密度，是被封鎖與否的唯一相關量；**並行度**只決定「同時有幾檔在路上」，讓單檔的網路往返（數百毫秒到數秒）不再逐檔累加到總耗時上。序列執行時總耗時是「每檔延遲 + 間隔」的總和，並行後總耗時由間隔決定而不再受延遲支配。

**並行度只對逐檔路徑（`SELECTED`）有意義。** 逐日路徑的請求數以交易日計，數量本來就少一個數量級，總耗時同樣由間隔決定；把它並行化只是讓多個工作者一起排隊等同一道閘門，換不到任何東西。逐日路徑循序執行即可。
...
**逐日快照來源（MI_INDEX）同樣套用「每來源獨立間隔」這套機制**，與 Yahoo／FinMind 各自計時、互不影響。設定值沿用同一組預設，不另訂一套。

##### 為什麼答案是「減少請求數」，不是「把速率調快」

這是本模組付出過代價才學到的一件事，寫在這裡以免下一個人再走一次：

全市場回補在改為逐日快照之前，總耗時的下限是 `母體檔數 × 每來源間隔`（1,030 × 0.5 秒 ≈ 8.6 分鐘），而且**這個數字與並行度無關**——間隔是全批共用的一道閘門，8 個工作者只是一起排隊等同一道閘門放行。當時可見的加速手段只剩兩個，兩個都是壞的：把間隔調小（直接提高被封鎖的機率，而封鎖的代價是整批停擺），或把並行度調大（對總耗時毫無作用）。

真正的出路是換一個請求數少一個數量級的來源。逐日快照把同一段區間的請求數從「檔數」降為「交易日數」，於是：

| | 逐檔（舊主路徑） | 逐日快照（新主路徑） |
|---|---|---|
| 一年份區間的請求數 | 約 1,030 | 約 165 |
| 每日收盤補一天 | 約 1,030 | **1** |
| 對來源施加的壓力 | 高 | **更低** |

它同時變快、同時降低被封鎖的機率——這是「減少請求數」相對於「調快速率」的根本差別，後者永遠是拿封鎖風險換時間。**任何未來的效能改動都先問「能不能少發幾次請求」，再考慮動間隔。**
...
逐日快照是 `ALL` 模式的**最佳化路徑，不是唯一路徑**。MI_INDEX 若對本機 IP 施加封鎖（HTTP 403／429，判定沿用上方「封鎖類回應」的同一條規則），處置與逐檔來源一致——標記該來源不可用並設定到期時間——但**後續行為不同**：

- **不結束本批作業**，改以逐檔路徑（Yahoo → FinMind）繼續處理剩餘標的。逐檔比較慢，但它會跑完。
- 只有在**逐日快照與兩個逐檔來源全部不可用**時，才適用上方「所有來源皆不可用」的處置（標的維持 `PENDING`、不累加 `attempt_count`、結束本批、留給下一次）。
- 冷卻到期後，下一次作業重新從逐日快照開始，不需人工介入。

降級不改變任何已處理標的的狀態，也不重抓已寫入的日期——`last_synced_date` 記的是「處理到哪一天」，逐檔路徑接著從次日繼續即可（見「斷點續傳」）。

**降級是靜默的效能變化，不是靜默的正確性變化**：兩種抓法的價格一致，但成交量口徑不同（見「價格處理」），因此降級期間寫入的列其 `source` 會是 `YAHOO`／`FINMIND` 而非 `TWSE`。`source` 欄位存在的意義就在這裡——事後要辨認一列是哪個口徑，看它就夠了。
...
- **交易所是成交量的正確口徑；Yahoo 是偏低的那一個。** 同一組比對中，成交量 163 天**全部不同**，Yahoo 系統性偏低，量比中位數 `0.864`（範圍 `0.54`～`0.95`），推測不含盤後定價、零股與鉅額交易。交易所給的是全額成交量。因此 `ALL` 模式改以逐日快照為主路徑之後，成交量是**變準，不是變不準**——這一點必須寫清楚，因為「換來源」直覺上聽起來像是拿準確度換速度，這裡剛好相反。
- **口徑差異會影響型態掃描的結果**：MACD／KD 與產業別漲幅排行（`specs/backend/industry-gain-ranking.md`）只讀價格，但型態掃描（`specs/backend/strategy-scan.md`）有兩類讀量——箱型突破的量能倍數（同一檔相鄰日相比，口徑台階只在切換點附近造成誤判），以及三個法人籌碼型態的佔比與強度（以成交量為分母，落在 Yahoo 口徑列上的比率偏高約一成多，命中與名次隨之偏移）。其餘用途只是顯示欄位（`latestVolume` / `totalVolume` / `avgVolume` 與日 K 圖的量副圖）。因此下一條的一次性重補不只是讓畫面整齊，也是讓法人籌碼型態建立在同一口徑上的前提。
- **既有由 Yahoo 寫入的歷史列與新列口徑不同，同一檔的成交量會在切換點出現一階台階。** 消除方式**不需要任何新機制**：以預設參數（`catchUp` 與 `resume` 皆為 `false`，即「重置進度重跑」）對整段歷史區間打一次全市場回補即可。該模式走逐日快照、UPSERT 覆蓋既有列，成本是區間的交易日數（一年份約 165 次請求，約 1.5 分鐘）。這是一次性動作，做完之後全庫同一口徑。
- **成交金額與成交筆數：交易所來源提供，Yahoo 不提供。** Yahoo 只給 OHLCV，由它取得的列 `turnover` 與 `transaction_count` 寫入 `0`；交易所快照（`STOCK_DAY_ALL` 與 MI_INDEX）與 FinMind 照常寫入實際值。上一條的重補因此**順帶修好這兩欄**：現存由 Yahoo 寫入的列在這兩欄都是 `0`，重補後會換成實際值。這是重補值得做的另一個理由，不只是為了量的一致性。

### 寫入語意

- 一律使用 UPSERT。重跑任一日或任一檔必須為冪等操作，且能修正資料源事後更正的值。
- 單檔的一次回補應在一個交易邊界內完成價格寫入與該檔進度更新，避免「資料已寫入但進度未更新」導致重跑時重複請求外部 API。
- **每日增量寫 `stock` 主檔時，內容沒變的股票不寫。** 先以**一次查詢**取回本次快照涵蓋之代號在 `stock` 的既有列，在記憶體中比對；只有「`stock` 中不存在」或「本路徑會寫入的任一欄位值與既有值不同」（實務上即名稱變更）的股票才寫入 `stock`。其餘一律不送任何寫入語句。當日 K 線寫入 `stock_daily_price` 與 `last_synced_date` 的推進**不受影響**，仍涵蓋快照中的每一檔。
  - 理由：主檔的代號與名稱幾乎天天相同，每日增量真正的新資料是當日行情。逐檔 UPSERT 一份沒變的名稱，每檔都是一次資料庫往返；資料庫在遠端時（實測每次往返約 205 毫秒），約 1,370 檔未變動的名稱就讓啟動主檔同步耗時 19 分鐘，而本節其後的日線補齊必須等它完成。
  - 比對查詢以本次快照的代號為條件；快照為空時不發出該查詢（沿用「目標清單為空」不得把空集合交給資料庫的規則）。
...
| 模式 | 抓法 | 依據 |
|---|---|---|
| `SELECTED`（提供 `stockIds`） | 逐檔歷史查詢（Yahoo 主／FinMind 備援） | 指名 N 檔的請求數為 N，遠少於區間的交易日數 |
| `ALL`（省略 `stockIds`） | 逐日全市場快照（MI_INDEX） | 請求數為交易日數，遠少於母體檔數 |

兩者共用同一套進度追蹤、速率控制、重試邏輯與 UPSERT 冪等性，**差別僅止於向外部要資料的方式**。抓法的選擇是純粹的成本判斷，不改變任何對外語意：同一段區間、同一批標的，兩種抓法寫進 `stock_daily_price` 的價格必須相同（實測依據見「價格處理」）。
...
母體與區間解析完成後，作業以**交易日**為單位推進：

1. 由 `startDate`／`endDate` 展開候選日期清單。清單以**台北日曆日**逐日展開；**不預先排除週末假日**——哪一天是交易日由資料源回答，在本機維護一份假日表只會多出一份需要每年更新、且錯了不會有人發現的資料。
2. 逐日請求 MI_INDEX，日期之間套用該來源的請求間隔（見「速率控制」）。
3. 取回應中 `fields[0]` 為 `"證券代號"` 的那一張表：
   - **表不存在，或其 `data` 為空** → 該日為非交易日或資料尚未發布。**視為正常處理完畢**：不寫入任何列、不記任何失敗、不累加 `attempt_count`，照常推進到下一日（`last_synced_date` 的推進見下方第 5 點）。這與「不得補零」是同一條原則的兩面——沒有交易就沒有列。
   - **表存在且有資料** → 進入第 4 點。
4. 正規化每一列，**只寫入本次目標母體內的標的**：母體外的代號（ETF、特別股、TDR，以及主檔中不存在的代號）一律丟棄，不寫入 `stock_daily_price`，也不新增 `stock` 主檔列。逐日快照是**行情**來源，不是 universe 來源；universe 由 `specs/backend/stock-universe-import.md` 與每日增量維護，多一條寫主檔的路徑正是分類與 `is_active` 最容易悄悄不一致的成因。`source` 寫入 `TWSE`。
5. 該日成功處理後（含第 3 點的非交易日情形），把**本次目標母體內所有標的**的 `last_synced_date` 推進到該日。這是斷點續傳的唯一依據：作業中斷後，下一次從最後一個成功日的次日接續，不重抓已處理的日期。
6. 全部日期處理完畢後，把目標母體內所有標的的進度標記為 `DONE`。

**單日的寫入與進度推進必須在同一個交易邊界內完成**，理由與逐檔路徑相同：避免「資料已寫入但進度未推進」導致重跑時重複請求外部 API。

**進度的顯示語意刻意維持逐檔不變。** `stock_sync_progress` 仍是每檔一列，`GET /api/stocks/sync/progress` 仍回逐檔的 `pending`／`running`／`done`／`failed`／`skipped` 計數，`targetCount` 與 `caughtUpCount` 仍是檔數。抓法是本模組內部的成本決策，不該外溢成呼叫端要重新理解的一套新語意——`specs/frontend/strategy.md` 的同步列因此完全不需要改動。代價是逐日模式下逐檔計數會在最後一天集中由 `PENDING` 轉為 `DONE`，中途看起來像沒有進展；這可以接受，因為這條路徑的總耗時本來就從數分鐘降到數十秒甚至一次請求，而該畫面對「瞬間完成」的情形已有明確規範（完成摘要必須持續顯示）。

### 斷點續傳

- 批次啟動時，為目標標的在 `stock_sync_progress` 以 `job_type = 'PRICE_BACKFILL'` UPSERT 建立 `PENDING` 列。
- `SELECTED` 模式逐檔處理，`ALL` 模式逐日處理；兩者的狀態皆依 `specs/dba/stock-sync-progress.md` 的進度語意流轉。
...
- 逐日模式的續傳粒度是**天**：中斷時已處理到第 D 天，則所有目標標的的 `last_synced_date` 均為 D，下一次從 D 的次日接續。逐日快照涵蓋全母體，因此不存在「有些檔補到 D、有些檔補到 D-3」的參差狀態——這比逐檔模式的續傳更簡單，不是更複雜。

### 寫入行情的路徑都必須推進進度

**任何成功把某交易日的行情寫進 `stock_daily_price` 的路徑，都必須把它涵蓋到的標的的 `last_synced_date` 推進到該交易日。** 這包含每日增量與逐日快照，不只回補。

這條規則存在的原因是一個實際發生的浪費：啟動時的主檔同步等同於呼叫每日增量服務（見「啟動時同步股票主檔」），而每日增量**已經把當日的 OHLCV 寫進 `stock_daily_price`**；但每日增量「不需要進度追蹤」故不碰 `stock_sync_progress`，而 `catchUp` 的跳過條件只讀 `stock_sync_progress.last_synced_date`。結果是資料明明已經在庫裡，緊接著執行的啟動補齊仍判定每一檔都落後一天，把整個母體重新開啟再抓一次——**每個交易日重演一次**。

具體規範：

- 每日增量成功寫入後，對 `job_type = 'PRICE_BACKFILL'` **既有的**進度列，把 `last_synced_date` 推進到該次快照的交易日。
- 推進的日期取自**回應中實際的交易日**，不是系統當下的日期。快照在盤中或資料尚未發布時回的是前一個交易日，用系統日期會記下一個從未被處理過的日期，而 `catchUp` 會據此永久跳過那一天。
- 只在既有值早於該交易日時推進，不倒退。
- **只更新既有列，不新建列。** 新標的的進度列一律由回補路徑建立——每日增量涵蓋約 1,378 檔（含非普通股），若由它建列，進度表的 `total` 會從母體檔數膨脹到快照檔數，而該數字是呈現在畫面上的。
- 每日增量的 API 回應契約不變，不新增欄位。推進進度是它既有職責的內部延伸，不是新的對外行為。
...
- **可停用**，與啟動補齊各自獨立開關。
- **順序固定：主檔同步在前，日線補齊在後。** 反過來會讓本次啟動新增進來的股票要等到下一次重啟才補得到行情。主檔同步是單一請求、耗時以秒計，讓補齊等它完成不違反「絕不阻塞啟動」——真正不能同步等待的是後面那條長度取決於待補區間的回補作業。
...
- **絕不阻塞啟動。** 補齊在背景非同步執行，應用程式必須在補齊開始後立刻進入可服務狀態。改為逐日快照後，穩態下的啟動補齊通常只需個位數次請求（見「寫入行情的路徑都必須推進進度」），但一個全新的資料庫仍要補滿整段起始區間；補齊耗時取決於區間長度而非母體大小，不得因為常見情形很快就改成同步等待。
...
- **重啟安全。** 開發過程中重啟極為頻繁，因此重啟不得重抓已有的資料。`catchUp` 的跳過條件即是這件事的保證：已補到今日的檔在下次啟動時完全不發出外部請求。這條保證能否成立，取決於每日增量是否推進了進度——見「寫入行情的路徑都必須推進進度」，那條規則正是為了讓這裡寫的話成為事實。
...
**每日增量**：取得全市場快照 → 正規化每列（代號、名稱、日期、價格）→ 一次查詢取回既有主檔列並比對 → 只對新股票與內容有變的股票寫入 `stock` → UPSERT `stock_daily_price` → 推進既有進度列 → 回傳統計。`stockMasterUpserted` 的值維持為本次快照涵蓋的主檔檔數（含未變動而略過寫入者），回應契約不變。單一請求即涵蓋全市場，不需進度追蹤。

**回補（`SELECTED`）**：解析標的清單 → 初始化 `stock_sync_progress` → 逐檔依速率限制請求歷史 → 正規化並 UPSERT → 更新該檔進度 → 全部完成後結束。任一檔失敗只影響該檔進度，不中止整批。

**回補（`ALL`）**：解析母體 → 初始化 `stock_sync_progress` → 展開交易日清單 → 逐日依速率限制請求 MI_INDEX → 正規化並 UPSERT 母體內標的 → 推進全母體的 `last_synced_date` 到該日 → 全部日期完成後標記 `DONE`。完整規範見「逐日回補的處理流程」；逐日快照不可用時降級為逐檔，見「逐日快照不可用時降級為逐檔」。
...
**每日增量的推進是另一回事，且刻意不遵守這條。** 它推進到的是快照回應中的**實際交易日**（見「寫入行情的路徑都必須推進進度」），因為它沒有「請求區間」可言——它只知道自己處理了哪一天。這確實會在週末重新打開上面那個缺口：週六啟動時每日增量拿到的是週五的資料，`last_synced_date` 記為週五，而啟動補齊的 `endDate` 是今日（週六），於是整個母體被重新開啟。

**這個缺口現在無害，不需要為它加任何機制。** 重新開啟後走的是逐日快照路徑，區間為週六到週六——**1 次請求**，回應中沒有個股行情表（非交易日），照常推進 `last_synced_date` 到週六後結束。成本從「1,030 次請求」變成「1 次請求」，這正是把成本單位從檔數換成天數之後的直接結果：續傳判斷失準的代價，從此以「多發幾次請求」計，而不是以「多發幾百次請求」計。用一份本機假日表去精確預測哪一天不用抓，換來的節省是個位數次請求，卻要承擔一份沒人會更新的資料。不值得。
...
- [x] 同一端點省略 `stockIds` 時回應 `mode` 為 `ALL`，母體為 `is_active = 1` 且通過普通股篩選的股票
...
- [x] 回補過程對外部資料源的請求有間隔控制；逾時與 5xx 等非封鎖類失敗觸發指數退避重試而非立即失敗（`403`／`429` 屬封鎖類回應，改走來源切換，見下節）
...
- [x] 任一來源被判定封鎖後，其餘工作者取下一檔時即不再對該來源發出任何請求（封鎖狀態為全批共用、非每工作者一份；以請求數斷言，非以耗時推測），直到封鎖到期
...
- [x] 承上，停止的是「領取新標的」，已在處理中的標的照常收斂完成後作業才回報結束
