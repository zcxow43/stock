...
本頁同時定義**共用的圖表元件**：分 K 頁沿用同一個元件的座標軸、十字準星與點選行為，只是餵入不同週期的資料。兩頁各自實作一份座標軸與互動，最終必然在配色、十字準星行為上出現不一致。數值的呈現方式由各頁決定：本頁以圖表上方的固定資訊列呈現（見「資訊列」），分 K 頁仍使用元件提供的浮動 tooltip。
...
K 棒的逐日數值顯示在**圖表面板頂端的一條固定資訊列**，而不是跟著滑鼠移動的浮動資訊框。內容由左至右為：

日期、開、高、低、收、漲跌額與漲跌幅（依漲跌著色）、成交量（張）、`DIF`／`DEA`／`OSC`、`K`／`D`／`J`。

- 顯示的是**十字準星所在的那一根**：已釘選時為釘選的 K 棒；未釘選時為滑鼠目前所在的 K 棒；滑鼠不在圖上且未釘選時，顯示區間內**最新一根**。資訊列因此永遠有內容，高度固定，不會因滑鼠進出圖表而出現或消失、推動下方圖表。
- 資訊列位置固定，只有數值隨十字準星更新。
- 寬度不足時換行，不得產生水平捲軸。每一項數值**預留固定寬度**（依該項最長可能值），換行位置因此不隨數值長短改變——否則指標為「—」的日期比有數值的日期少一行，滑鼠掃過時下方圖表會上下跳動。
- 指標為 `null` 時該項顯示「—」。價格 2 位小數、指標 4 位小數。

為什麼不用浮動資訊框：浮動框貼著十字準星移動，會蓋住使用者正要看的那幾根 K 棒與副圖（MACD／KD 的線段正好在框的下方），而且每移一格位置就跳一次，眼睛得一直追著框找數字。固定在圖表上方，數字永遠在同一個位置，圖表本身不被遮擋。
...
| 操作 | 行為 |
|---|---|
| **單擊**一根 K 棒（或其對應的 X 軸位置） | 釘選該根 K 棒——十字準星與資訊列固定在該日，不再隨滑鼠移動；同時該根 K 棒加上選取外框。**不導覽、不改變網址。** |
...
| 資訊列背景／下邊框 | `#1B2836` / `#26333F` |
| 資訊列日期與數值文字 | `#E6EDF5` |
| 資訊列項目標籤（開、高、成交量…） | `#93A4B8` |
...
- [x] Tooltip 顯示該日的日期、開高低收、漲跌額與漲跌幅、成交量、`DIF`/`DEA`/`OSC`、`K`/`D`/`J`，價格 2 位小數、指標 4 位小數（已由下方「資訊列」取代，內容與格式不變）
- [x] 圖表面板頂端有固定資訊列，顯示十字準星所在那一根的日期、開高低收、漲跌額與漲跌幅、成交量、`DIF`/`DEA`/`OSC`、`K`/`D`/`J`；滑鼠移動時資訊列位置不變、只有數值更新，圖表上不出現跟隨滑鼠的浮動資訊框
- [x] 未釘選且滑鼠不在圖上時，資訊列顯示區間內最新一根的數值，不會消失
...
- [x] **單擊一根 K 棒只釘選該日的十字準星與資訊列，網址不變、不發生導覽**
