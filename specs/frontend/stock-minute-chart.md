...
沿用 `specs/frontend/stock-daily-chart.md` 定義的圖表元件的**座標軸、十字準星與單擊釘選**行為，並**傳入浮動 tooltip**（日 K 頁改用圖表上方的固定資訊列、不傳浮動 tooltip；本頁維持浮動 tooltip 不變）：本頁傳入的 X 軸標籤為 `HH:mm`、副圖僅一個成交量圖、不傳入雙擊回呼（分 K 沒有下一層可鑽）。
