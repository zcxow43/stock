...
| MACD 黃金交叉 | `{策略名稱}（{fastPeriod}／{slowPeriod}／{signalPeriod}）` | MACD 黃金交叉（12／26／9） |
...
### 技術指標卡片（本次新增）
- [ ] 條件區出現「MACD 黃金交叉」「KDJ 黃金交叉」兩張卡片，依 API 順序排在法人籌碼卡片之後；名稱、說明文字、各參數的標籤／單位／預設／範圍皆取自 `GET /api/strategies`；兩張皆沒有靈敏度下拉；前端程式中不存在以 `MACD_GOLDEN_CROSS`、`KDJ_GOLDEN_CROSS`、`fastPeriod` 或 `jThreshold` 判斷的分支
- [ ] 十張卡片一列放不下時換行，頁面不出現水平捲軸，卡片內輸入不被截斷
- [ ] MACD 卡片有「短期 EMA」（預設 12）與「長期 EMA」（預設 26）兩格，後綴「日」，下方有「回看的交易日數，不含週末與休市日」說明行
- [ ] KDJ 卡片有一格「J 門檻」（預設 40），**無後綴**（`unit` 為空字串，不畫空白後綴框）；可輸入負數，`-12.5` 為合法值且送出為 `-12.5`
- [ ] J 門檻輸入 `-100.1`、`100.1`、`40.25` 時卡片下方顯示「J 門檻需介於 -100 ~ 100」並不送出請求；`-100`、`100` 可送出
- [ ] 短期 EMA 輸入 `1`、`51`、`12.5` 時卡片下方顯示「短期 EMA需介於 2 ~ 50 的整數」並不送出；長期 EMA 輸入 `2`、`101` 時顯示「長期 EMA需介於 3 ~ 100 的整數」
- [ ] `lessThan`：短期 EMA ≥ 長期 EMA（`26／26`、`30／20`）時卡片下方顯示「短期 EMA需小於長期 EMA」並擋下送出；改動任一格使其成立時訊息即消失；任一格超出範圍時只顯示範圍錯誤。把 mock 回應中的 `lessThan` 移除後，同樣的輸入不再被擋（證明依 `lessThan` 判斷而非寫死）
- [ ] 送出的該筆：MACD 為 `{ code, fastPeriod, slowPeriod }`、KDJ 為 `{ code, jThreshold }`；皆不帶 `preset`、`risePercent`、`days`；未勾選時兩張卡片的輸入皆 disabled 且請求中不含該策略
- [ ] 後端回 `INVALID_FAST_PERIOD`／`INVALID_SLOW_PERIOD`／`INVALID_MACD_PERIODS`／`INVALID_J_THRESHOLD` 時，訊息顯示在回應 `strategy` 指名的卡片下方，而非全頁通用錯誤
- [ ] 本次採用參數那一行顯示「MACD 黃金交叉（12／26／9）」「KDJ 黃金交叉（前一日 J < 40）」，數字取自掃描回應而非畫面輸入（`signalPeriod` 亦取自回應）；回應 `jThreshold: -10.0` 顯示為「前一日 J < -10」
- [ ] 命中策略與訊號日欄顯示「MACD 黃金交叉 {signalDate}」「KDJ 黃金交叉 {signalDate}」；回測後買進日等於訊號日，且送出回測的 `buyDate` 取自掃描回應
- [ ] `insufficientData` 非空時顯示「{策略名稱}：另有 N 檔因區間前的歷史資料不足而未納入判定」；兩型態不出現待確認行
- [ ] 兩張卡片與上述文字不含「建議」「推薦」「可進場」等措辭；本次改動未新增任何顏色，所有新文字沿用 `## Visual Style` 既有色碼，`prefers-color-scheme: dark` 與 `light` 下一致
