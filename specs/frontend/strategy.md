...
| `presets` 為空陣列、帶 `params` | 累積上漲、反彈、法人買賣超佔比、法人連續買超、法人買超強度排名 | 勾選框、策略名稱、**依 `params` 逐一畫出的輸入**（數字輸入或複選勾選框，見下）、策略層級 `description` 的說明文字一行。**這種卡片沒有靈敏度下拉** |

卡片共八張，依 API 回應的順序排列，**一列放不下時換行**，不得產生水平捲軸，也不得為了塞進一列而截斷卡片內的輸入。
...
**`params` 驅動的輸入（累積上漲、反彈與三張法人籌碼卡片）**：卡片上每個參數依其 `type` 畫出。不帶 `type` 的是數字參數：畫一格數字輸入，標籤取自該參數的 `name`、後綴取自 `unit`（`日`、`%` 或 `名`），範圍與步進取自 `min`／`max`／`step`，初始值取自 `default`。**這些數字一個都不寫死在前端。**
...
| `INVALID_WINDOW_DAYS` / `INVALID_BUY_DAYS` / `INVALID_TOP_N` | 於回應 `strategy` 指名的那張卡片下方顯示「<參數名>需介於 <min> ~ <max> 的整數」（前端已先擋，此為後備），參數名、`min`、`max` 取自 `GET /api/strategies` 中該策略對應參數的 `name`／`min`／`max`，不在前端寫死數字 |
