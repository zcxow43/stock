...
### 法人籌碼卡片
- [ ] 條件區出現三張法人籌碼卡片「法人買賣超佔比」「法人連續買超」「法人買超強度排名」，名稱、說明文字、各參數的標籤／單位／預設／範圍皆取自 `GET /api/strategies`；三張皆沒有靈敏度下拉
- [ ] 八張卡片一列放不下時換行，頁面不出現水平捲軸，卡片內輸入不被截斷
- [ ] 複選參數依 `type: "multiSelect"` 畫成勾選框：每張卡片有「法人」標籤與「外資」「投信」兩個勾選框，初始皆勾選；把 mock 回應中某個數字參數改為 `multiSelect`，卡片跟著改畫勾選框（前端不存在以 `investors` 或策略 `code` 判斷的分支）
- [ ] 取消勾選到一個都不剩時，卡片下方顯示「請至少勾選一個法人」，且不送出請求；再勾回任一個即消失
- [ ] 送出的該筆只帶自己 `params` 列出的欄位：法人買賣超佔比為 `{ code, investors, windowDays, ratioPercent }`、法人連續買超為 `{ code, investors, buyDays }`、法人買超強度排名為 `{ code, investors, windowDays, topN }`；皆**不帶** `preset`、`risePercent`、`days`；`investors` 依外資、投信順序排列，只勾投信時為 `["TRUST"]`
- [ ] 數字參數超出範圍或非整數時，卡片下方顯示「<參數名>需介於 <min> ~ <max> 的整數」（佔比門檻為「佔比門檻需介於 0 ~ 100」），數字取自 `params`，不送出請求
- [ ] 後端回 `INVALID_WINDOW_DAYS`／`INVALID_RATIO_PERCENT`／`INVALID_BUY_DAYS`／`INVALID_TOP_N`／`INVALID_INVESTORS` 時，訊息顯示在回應 `strategy` 指名的卡片下方，而非全頁通用錯誤
- [ ] 命中策略與訊號日欄：法人連續買超與法人買超強度排名依 `detail.matchedInvestors` 顯示「（外資）」「（投信）」「（外資／投信）」；法人買賣超佔比另接方向，例如 `matchedInvestors: ["FOREIGN","TRUST"]`、外資 `BUY`、投信 `SELL` 顯示「法人買賣超佔比（外資買超／投信賣超） {signalDate}」；展開後的子列採同一格式
- [ ] 回測後買進日欄顯示掃描回應該筆的 `buyDate`，與訊號日不同（以 `signalDate: 2026-08-27`、`buyDate: 2026-08-28` 驗證兩欄各自顯示）；送出回測的 `buyDate` 取自回應，前端不自行加一日
- [ ] 本次採用參數那一行依格式表顯示三個型態的法人、參數與「法人資料至 {dataThroughDate}」，數字與日期取自掃描回應而非畫面輸入；`dataThroughDate` 為 `null` 時顯示「尚無法人資料」
- [ ] `pendingConfirm` 非空時，合併表格下方顯示「{策略名稱}：另有 N 檔已達標，但次一交易日尚未到」（提示色 `#D9A441`），這些代號不出現在合併表格、不送入回測
- [ ] `insufficientData` 非空時顯示「{策略名稱}：另有 N 檔因行情或法人資料不足而未納入判定」，五個價格型態的資料不足文字不變
- [ ] 三張卡片與上述文字不含「建議」「推薦」「可進場」等措辭；本次改動未新增任何顏色，所有新文字與勾選框沿用 `## Visual Style` 既有色碼，`prefers-color-scheme: dark` 與 `light` 下一致
