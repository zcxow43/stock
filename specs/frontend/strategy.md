...
每個策略一個區塊，標題括號內寫的是**這次實際用的參數**，取自該策略結果自己回報的欄位：有靈敏度的型態回 `preset`，標題為「{策略名稱}（{靈敏度}）— 命中 N 檔」；累積上漲回 `days`，標題為「累積上漲（{days} 日）— 命中 N 檔」。N 取自該策略結果的 `matchedCount`，其下為表格。
...
| `PRESET_NOT_APPLICABLE` / `DAYS_NOT_APPLICABLE` | 視為程式錯誤：正常操作不會送出這種組合（卡片依 `presets` 是否為空決定畫哪些控制項），顯示通用錯誤訊息 |
