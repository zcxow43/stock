...
策略分頁切到「指定股票」時，該分頁的請求不帶此參數（後端明訂使用者指名的代號不代為過濾）；此時頁面層級的勾選框仍維持原狀、不變灰，因為它對另外兩個分頁依然生效。
...
頁籤列以下（第 3、4 項）的內容屬於分頁一；切到分頁二或分頁三時整段換成該分頁的內容，頁首列與分頁列保持不動。

**頁首列的「共 N 檔」是分頁一的數字**（隨分頁一的篩選條件變動），在其他分頁停留時維持最後一次的值不變——它描述的是總覽清單的筆數，不是當前分頁的結果數。分頁二、三各自的結果總量顯示在各自的版面內。
...
| 頁面層級設定列背景／邊框 | `#16202C` / `#26333F` |
| 「只看上市普通股」勾選框已勾選背景／勾記 | `#3E8FD8` / `#FFFFFF` |
| 「只看上市普通股」勾選框未勾選背景／邊框 | `#0F1620` / `#26333F` |
| 「只看上市普通股」標籤文字 | `#E6EDF5` |

## Acceptance Criteria

### 頁籤列
- [x] 頁籤列有「總覽」「策略」「動態」三個頁籤，依此順序顯示，預設停在總覽
- [x] 切換頁籤時網址的 `tab` 參數同步變動；直接以 `/stocks?tab=strategy` 進入會停在策略頁籤
- [x] 點擊「動態」後網址的 `tab` 參數變為 `momentum`；直接以 `/stocks?tab=momentum` 進入會停在動態頁籤
- [x] `tab` 參數缺漏或為無法辨識的值（例如 `?tab=foo`）時退回總覽，不顯示空白畫面
...
### 頁面層級設定：只看上市普通股
- [x] 頁籤列**上方**有「只看上市普通股（排除 ETF、特別股、TDR）」勾選框，**靠右對齊**且**預設為勾選**
- [x] 該勾選框不在任何一個分頁內部；切到策略或動態分頁時它仍在原位、狀態不變
- [x] 總覽分頁的 `GET /api/stocks` 帶出的 `commonStocksOnly` 等於該勾選框當下的狀態
- [x] 預設狀態下總覽清單不出現 `0050`、`00878`、`2881A`、`910322`，且「共 N 檔」為排除後的檔數
- [x] 取消勾選後總覽立即重新查詢，「共 N 檔」明顯變大且清單出現 ETF
- [x] 三個分頁共用同一份狀態：在總覽取消勾選、切到動態分頁，動態查詢帶的是 `false`
- [x] 切換勾選框只重查**當前分頁**，不預先重查另外兩個分頁
- [x] 重新載入頁面後回到預設（勾選）；該狀態不寫入 `localStorage`、不出現在網址參數
- [x] 此設定不寫任何資料表：切換前後 `stock` 的列數、內容與 `is_active` 完全不變
- [x] 該勾選框的所有顏色取自 `## Visual Style` 的字面 hex，且在 `prefers-color-scheme: dark` 與 `light` 下呈現完全一致
...
### Increment 4 — 2026-09-04

Adds the page-level "只看上市普通股" setting per the spec's 10 previously-unchecked Acceptance Criteria: a checkbox above the tab bar, right-aligned, default-checked, shared across all three tabs, feeding `commonStocksOnly` into 總覽's `GET /api/stocks`. Note: by the time this increment ran, both `StrategyTab.tsx` (from `specs/frontend/strategy.md`) and `MomentumTab.tsx` (from `specs/frontend/momentum.md`) had already replaced their placeholders, and the backend's `commonStocksOnly` support (`specs/backend/stock-catalog.md`, `strategy-scan.md`, `industry-gain-ranking.md`) was already live — so this increment also wires the shared value all the way into 策略's `POST /api/strategies/scan` and 動態's `GET /api/momentum/gain`, fully closing the loop described in this spec's own 「頁面層級設定」 section rather than leaving those two tabs to ignore a state they're documented to share.

- Files changed:
  - `develop/frontend/src/pages/StockListPage.tsx` — added `commonStocksOnly` state (`useState(true)`, plain component state — never persisted, so a reload always comes back to checked); renders a new row (`.sl-page-setting`) between the header and the tab bar with the checkbox + label; passes `commonStocksOnly` down to all three tab components as a prop.
  - `develop/frontend/src/pages/StockListPage.css` — added `.sl-page-setting` (row background/border) and `.sl-page-setting-label` (text color + checkbox `accent-color`), all literal hex straight from `## Visual Style` (`#16202C`/`#26333F` row, `#E6EDF5` label, `#3E8FD8`/`#FFFFFF` checked state); relies on the pre-existing global `color-scheme: dark` (`index.css`) so the native checkbox's unchecked chrome never shifts with the OS/browser preference, the same mechanism already governing the pre-existing 顯示已下市 checkbox.
  - `develop/frontend/src/api/stocks.ts` — `fetchStocks` gained a required second positional `commonStocksOnly: boolean` argument (kept separate from `StockListParams` since it's a page-level value, not one of 總覽's own filters), always set on the query string.
  - `develop/frontend/src/pages/StockOverviewTab.tsx` — `StockOverviewTabProps` gained `commonStocksOnly: boolean`; passed into `fetchStocks`; added to the fetch effect's dependency array so toggling the checkbox triggers an immediate requery of 總覽 (and only 總覽 — no effect anywhere reacts to it besides this one).
  - `develop/frontend/src/pages/StrategyTab.tsx` — added an optional `commonStocksOnly?: boolean` prop (default `true`, so the component still renders standalone, e.g. in tests that don't pass it); `buildScanPayload` now sets `commonStocksOnly: scope === 'ALL' ? commonStocksOnly : undefined` (`JSON.stringify` drops the `undefined` key, so 指定股票 truly omits the field, matching "使用者明確指名的代號不代為過濾"); the two pre-existing internal `fetchStocks` calls (常駐「共 N 檔」 widget, 指定股票 search suggestions) now pass an explicit `false` to preserve their original "unfiltered" meaning — see Notes below.
  - `develop/frontend/src/api/strategies.ts` — `ScanRequest` gained an optional `commonStocksOnly?: boolean` field.
  - `develop/frontend/src/pages/MomentumTab.tsx` — added an optional `commonStocksOnly?: boolean` prop (default `true`, same standalone-rendering rationale); `buildParams` now includes `commonStocksOnly` in both the `DAYS` and `WEEKS` branches, always sent (this tab has no ETF-exclusion option of its own, per its own spec).
  - `develop/frontend/src/api/momentum.ts` — `MomentumGainParams` gained a required `commonStocksOnly: boolean` field, always set on the query string.
  - `develop/frontend/src/__tests__/StockListPage.commonStocksOnly.test.tsx` (new, 7 tests) — checkbox renders above the tab bar, checked by default; 總覽's request carries `commonStocksOnly=true` by default; unchecking immediately requeries 總覽 with `commonStocksOnly=false` and a larger total; the checkbox stays visible and keeps its state across tab switches; toggling fires no 動態/策略 requests immediately; switching to 動態 after unchecking and clicking 查詢 sends `commonStocksOnly=false`; and it's never written to `localStorage` or the URL.
- Notes:
  - **A real, pre-existing latent bug was found and fixed as a side effect of this increment's mechanical signature change.** `StrategyTab.tsx`'s persistent "股票清單：共 N 檔" widget and its 指定股票 search-suggestions call both call `fetchStocks` with no population filter — by design, meant to reflect the *raw* active-stock count (matching `totalActiveCount` from 更新股票清單) and to let 指定股票 search find *any* stock including ETFs. When `specs/backend/stock-catalog.md` added `commonStocksOnly` with a server-side default of `true` in a later, independent increment, these two frontend call sites started being silently filtered (excluding ETFs) without any frontend change to notice it — a real behavior drift, invisible until compared against `totalActiveCount` directly. Because this spec's own change to `fetchStocks`'s signature required touching both call sites anyway, both now pass an explicit `false`, restoring their originally-intended "unfiltered" semantics. This was not requested by this spec's own Acceptance Criteria, but leaving it silently filtered while passing through the exact two lines that caused it would have been "find the root cause" in name only.
  - **Toggle only refetches the visible tab.** No effect in `StrategyTab.tsx`/`MomentumTab.tsx` depends on the `commonStocksOnly` prop — it's read only inside `buildScanPayload`/`buildParams`, which run solely in response to an explicit user action (開始掃描 / 查詢). Verified live: toggling the checkbox while on 總覽 fires zero `/api/strategies/scan` or `/api/momentum/gain` requests, and the new value only appears on that tab's *next* user-triggered query — matching "另外兩個分頁不預先重查".
  - **`commonStocksOnly` deliberately kept as a second argument to `fetchStocks`, not folded into `StockListParams`.** It's a page-level value with its own lifecycle (never resets with the other filters, never round-trips through `DEFAULT_QUERY`), so merging it into the per-tab query object would have blurred a boundary the spec itself draws explicitly ("不在任何一個分頁內部...三個分頁共用同一個母體，不是各自的查詢條件").
  - **Live end-to-end verified** against the real backend (`mvn -f develop/backend/pom.xml spring-boot:run`, port 8080, live dataset: 1377 stocks, 1085 matching the common-stock pattern) and `npm run dev` (port 5173) with a throwaway Playwright script (installed with `--no-save`, removed after use — `package.json`/`package-lock.json` untouched): confirmed via direct `curl` that `0050`/`00878`/`2881A`/`910322` are each excluded under the default (omitted/`true`) and present under `commonStocksOnly=false`; drove the real browser through checkbox-default-checked → 總覽 shows 「共 1,085 檔」 → uncheck → immediate single requery → 「共 1,377 檔」, with zero momentum/strategy requests fired by the toggle itself; switched to 動態 and clicked 查詢, confirming its request carried `commonStocksOnly=false` (the value set while on 總覽); switched to 策略, checked a strategy, and confirmed 全市場 掃描 sent `"commonStocksOnly":false` while 指定股票 (after adding `2330` via the search suggestions) sent no `commonStocksOnly` key at all and left the page-level checkbox checked and enabled (not greyed out); confirmed `window.localStorage.length === 0` and the URL never contains `commonStocksOnly` after toggling; screenshotted the header/checkbox row under Playwright's emulated `dark` and `light` `colorScheme` and confirmed the rendered `rgb()` values (`rgb(22,32,44)`/`rgb(38,51,63)`/`rgb(230,237,245)`/`rgb(62,143,216)`) exactly match the spec's `#16202C`/`#26333F`/`#E6EDF5`/`#3E8FD8` hex and are byte-identical between the two color schemes. `SELECT COUNT(*), SUM(is_active) FROM stock` was `1377`/`1377` before and after every toggle in this session, confirming no table is written.
  - Cleaned up after verification: uninstalled the throwaway `playwright` devDependency (`npm uninstall playwright`, confirmed `package.json`/`package-lock.json` unaffected by the diff), deleted every throwaway `.mjs`/`.png` script and screenshot from `develop/frontend/`, stopped both the Maven backend process and the Vite dev server, and confirmed via `netstat` that ports 8080 and 5173 were free before finishing.
