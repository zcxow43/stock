...
擺在**頁籤列的上方**，不在任何一個分頁內部——它決定的是「這個系統現在看的是哪一批股票」，三個分頁共用同一個母體，不是各自的查詢條件。放進任一分頁會讓另外兩個分頁的行為變成一個看不見的副作用。
...
- [ ] 頁籤列**上方**有「只看上市普通股（排除 ETF、特別股、TDR）」勾選框，**預設為勾選**
- [ ] 該勾選框不在任何一個分頁內部；切到策略或動態分頁時它仍在原位、狀態不變
- [ ] 總覽分頁的 `GET /api/stocks` 帶出的 `commonStocksOnly` 等於該勾選框當下的狀態
- [ ] 預設狀態下總覽清單不出現 `0050`、`00878`、`2881A`、`910322`，且「共 N 檔」為排除後的檔數
- [ ] 取消勾選後總覽立即重新查詢，「共 N 檔」明顯變大且清單出現 ETF
- [ ] 三個分頁共用同一份狀態：在總覽取消勾選、切到動態分頁，動態查詢帶的是 `false`
- [ ] 切換勾選框只重查**當前分頁**，不預先重查另外兩個分頁
- [ ] 重新載入頁面後回到預設（勾選）；該狀態不寫入 `localStorage`、不出現在網址參數
- [ ] 此設定不寫任何資料表：切換前後 `stock` 的列數、內容與 `is_active` 完全不變
- [ ] 該勾選框的所有顏色取自 `## Visual Style` 的字面 hex，且在 `prefers-color-scheme: dark` 與 `light` 下呈現完全一致
