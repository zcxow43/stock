...
- [backend](blueprint/backend.md) — 整合 `stock-price-ingestion`、`stock-indicator-statistics`、`stock-catalog`、`stock-minute-price` 與 `strategy-scan`：從啟動自動補齊、行情入庫、指標推導、策略型態掃描，到前端清單／日 K／分 K 讀取的完整系統圖景。大方向／圖表導向——欄位、限制條件與 API 契約細節見下方 `docs/backend/` 各文件。
