...
| [strategy](strategy.md) | 更新股票清單 → 同步日 K → 勾選八個策略（含三個法人籌碼型態）掃描（單一命中彙總表）並自動回測 → 點「報酬率」表頭排序 → 展開一檔的多個買進日 → 取消其中一筆重算總計（排序位置不動） → 勾選「取消全選」 → 全部勾回 → 勾選「取消買進價高於 500 元」 → 往下捲動、三個總計浮動跟隨 | 13 |
